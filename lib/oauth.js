...
var OAuth = function (appid, appsecret, getToken, saveToken) {
  ...
  console.log('OAuth');
  // this.request = require('./request').module;
  require('./request').module({url: 'http://bing.com'}, function(err, res, data) {
        console.log(',,data');
    if (err) {
      console.log(err);
    } else {
...
OAuth.prototype.getAccessToken = function (code, callback) {
  ...
  require('./request').module({
    ...
    json: true,
    qs: info
  }, wrapper(processToken(that, callback)));
};
...
OAuth.prototype.getUser = function (openid, callback) {
  var that = this;
  this.getToken(openid, function (err, res, data) {
    if (err) {
      return callback(err);
    }
    // 没有token数据
    ...
    if (token.isValid()) {
      ...
      that.refreshAccessToken(token.data.refresh_token, function (err, res, token) {
        if (err) {
          return callback(err);
        }
