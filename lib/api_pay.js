 ...
 */
exports.deliverNotify = function (package, callback) {
 ...
 * 发货通知的未封装版本
 */
exports._deliverNotify = function (package, callback) {
  var url = this.payPrefix + 'delivernotify?access_token=' + this.token.accessToken;
  require('./request').module({url: url, json: true, method: 'POST', body: JSON.stringify(package)}, wrapper(callback));
};
...
exports._orderQuery = function (query, callback) {
  ...
  require('./request').module({url: url, json: true, method: 'POST', body: JSON.stringify(query)}, wrapper(callback));
};
