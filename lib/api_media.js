...
exports._uploadMedia = function (filepath, type, callback) {
  ...
  fs.stat(filepath, function (err, stat) {
    ...
    var r = require('./request').module({url: url, json: true, method: 'POST', timeout: 60000}, wrapper(callback));
    var form = r.form();
    form.append('media', fs.createReadStream(path.join(filepath, 'media')));
...
exports._getMedia = function (mediaId, callback) {
  ...
  require('./request').module({url: url}, wrapper(function (err, data, res) {
    // handle some err
    if (err) {
      return callback(err);
    }
